 ...
 * Analysis component
 * 
 * This component renders the Analytics page only if the user is authenticated.
 ...
 * with the text "Analytics Page".
 */
const Analysis = () => {
  const isAuth = useSelector((state) => state.auth.isAuthenticated);
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('Analytics Table'); // State to manage the active component
  ...
  const renderActiveComponent = () => {
    if (activeTab === 'Analytics Table') {
      return <AnalyticsTable isAuth={isAuth} />;
    }
      ...
      {isAuth && (
        <div className="flex h-screen">
          ...
          >
            ...
            {/* Tab Navigation */}
            <div className="bg-white w-[200px] rounded-t-3xl flex justify-start ml-[270px]">
              <div className="w-[200px] h-[60px] rounded-t-3xl bg-white">
                <button
                  className={`px-4 w-full h-full flex justify-center py-2 rounded-t-3xl ${activeTab === 'Analytics Table' ? 'bg-indigo-500 text-white' : 'bg-white text-black'}`}
                  onClick={() => setActiveTab('Analytics Table')}
                >
            ...
            <div className="ml-[250px] w-[1200px] h-[600px] bg-white rounded-2xl p-4">
              {/* Content Area */}
